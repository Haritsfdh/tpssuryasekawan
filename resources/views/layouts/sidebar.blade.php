          ...
          @can($item->permission)            
            <li class="nav-item">
              <a href="{{ $linkActive }}" class="nav-link {{ ($pat == $linkActive) ? 'active' : '' }}" target="{{ $item->target }}">
                <i class="{{ $item->icon_class ?? 'far fa-circle'}} nav-icon"></i>
            ...
            @else
              ...
              <li class="nav-item">
                <a href="{{ $subActive }}" class="nav-link {{ ($pat == $subActive) ? 'active' : '' }}" target="{{ $subItem->target }}">
                  <i class="{{ trim($subItem->icon_class ?? 'far fa-circle') }} nav-icon "></i>
          ...
          @else
            ...
            <li class="nav-item {{ ($pat == $subActive) ? 'menu-open' : '' }}">
              <a href="{{ $subActive }}" class="nav-link">
                <i class="nav-icon {{ trim($subItem->icon_class ?? 'far fa-circle') }}"></i>
                ...
                @foreach($subItem->children->where('active', true) as $deepSub)
                  ...
                  <li class="nav-item">
                    <a href="{{ $deepSubActive }}" class="nav-link {{ ($pat == $deepSubActive) ? 'active' : '' }}" target="{{ $deepSub->target }}">
                      <i class="nav-icon {{ trim($deepSub->icon_class ?? 'far fa-circle') }}"></i>
